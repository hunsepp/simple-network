...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response{
    ...
    return shim.Error("Invalid Smart Contractfunction name.")
}
...
//state ok
func (s*SmartContract) addRatig(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	if len(args !=3{
		return shim.Error("INcorrect number fo argument,s expecing 3")
	}
	//getSTate User
	...
	} else if userAsBytes == nil{ // nostate!error
		jsonResp := "\"Error\":\"User does not exist :"+ args[0]+"\"}"
		return shim.Error(jsoinResp)
	}
